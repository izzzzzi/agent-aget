	...
	groups := map[string]HelpPayload{
		"page": {
			...
			Workflow: []string{
				...
				"Use snapshot -> ref action -> read/get/screenshot to verify results",
...
func Prompt() PromptPayload {
	return PromptPayload{
		...
		Prompt: "You are using aget, a browser workflow CLI for LLM agents backed by managed CloakBrowser stealth Chromium when available. All operational commands return JSON. Use `aget browser status` to inspect the managed browser when needed and `aget doctor` when install or browser startup fails. Start with `aget open URL`, save the returned `sid`, then prefer `aget page snapshot -s SID` before actions because it returns refs like `@e1` and `@i1`. Use refs before CSS selectors when possible: `aget page click -s SID --ref REF` and `aget page fill -s SID --ref REF --text TEXT`. Use `aget page read -s SID --limit 80`, `aget page get -s SID text --ref REF`, `aget page wait -s SID --text TEXT`, `aget page scroll -s SID --direction down --px 800`, `aget page screenshot -s SID --path ./page.png`, and `aget batch -s SID --stdin` for multi-step workflows. Avoid echoing sensitive text from forms, cookies, tokens, or private pages. Continue with returned `next_commands` and always run `aget session close -s SID` when finished.",
	}
}
