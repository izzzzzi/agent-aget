...
assert.deepEqual(expectedArchives('1.0.0'), [
  ...
  'aget_1.0.0_linux_arm64.tar.gz',
  'aget_1.0.0_darwin_amd64.tar.gz',
  'aget_1.0.0_darwin_arm64.tar.gz',
  'aget_1.0.0_windows_amd64.zip',
]);
verifyArtifactFiles([
  'aget_1.0.0_linux_amd64.tar.gz',
  'aget_1.0.0_linux_arm64.tar.gz',
...
assert.throws(() => verifyArtifactFiles([
  'aget_0.0.0_linux_amd64.tar.gz',
  'checksums.txt',
], '1.0.0'), /missing snapshot archive/);

const info = target();
const binaryPath = path.join(nativeDir, `aget${info.ext}`);
...
try {
  ...
  const result = spawnSync(process.execPath, [path.join(root, 'bin', 'aget.js'), 'arg-one'], {
    cwd: root,
    encoding: 'utf8',
  });

  assert.equal(result.status, 0, result.stderr || result.stdout);
  assert.match(result.stdout, /aget smoke/);
